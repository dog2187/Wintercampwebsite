...
const BLADE_SIZE = 500;

const BLADE_LENGTH = 0.1;

const BAD_FRUIT_PROBABILITY = 0.6;
...
function draw() {
    background(51, 250, 23, 0); //background color of map
clear();
    handleMouse();
    score += handleFruit();
...
// Send out new fruit
  if (frameCount % 10 === 0) {
		...
		if (noise(frameCount) > 0.3) { //decides how many times to send a fruit
...
// fruit cutting
	...
	for (var i = fruit.length - 1; i >= 0; i--) {
		...
		if (!fruit[i].visible) { // if fruit is no longer on the screen
			...
			points += (sword.checkForSlice(fruit[i])) ?438 : 0; // if fruit is cut, add 1 
		}
